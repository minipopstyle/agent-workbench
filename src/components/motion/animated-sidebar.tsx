...
export function AnimatedSidebarMenuButton({
    ...
    <>
      {isActive ? (
        <motion.span
          ...
          className="absolute inset-0 rounded-xl bg-white shadow-[0_1px_3px_rgba(15,23,42,0.1),0_1px_2px_rgba(15,23,42,0.06)] border border-slate-200"
        />
      ) : null}
      {icon ? (
        <span
  ...
  const interactiveClassName = cn(
    "relative flex min-h-9 w-full min-w-0 items-center gap-2.5 overflow-hidden rounded-xl px-3 text-left text-[14px] leading-5 font-medium outline-none",
